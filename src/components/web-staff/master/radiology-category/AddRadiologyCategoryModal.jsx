...
import { createRadiologyCategory } from '../../../../api/master-data-services/radiology-category';

export const AddRadiologyCategoryModal = ({ isOpen, onClose }) => {
    ...
    onSettled: async (data, error) => {
      ...
      if (data) {
        ...
        toast({
          position: 'top-right',
          title: 'Success',
          description: `Radiology Category berhasil ditambahkan`,
          status: 'success',
          duration: 4000,
          isClosable: true,
        });
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} size="lg">
      <ModalOverlay />
      <ModalContent>
        <ModalHeader>Create New Radiology</ModalHeader>
        <ModalCloseButton />
        <ModalBody>
          <Box mb="2">
            {fields.map(({ id }, index) => {
