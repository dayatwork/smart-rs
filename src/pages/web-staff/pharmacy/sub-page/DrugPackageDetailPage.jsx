...
export const DrugPackageDetailPage = () => {
  ...
  if (dataDrugPackageDetail?.code === 404 || dataDrugPackageDetail?.code === 400) {
      ...
      <Box>
        ...
        >
          ...
          <Box p="4">
            ...
            <Description
              title="Tanggal"
              value={new Date(dataDrugOrderDetail?.data?.created_at).toLocaleDateString(
                'id-ID',
                {
                  day: '2-digit',
                  month: 'long',
            ...
            <Box key={drug.id}>
              <Box mb="6" border="1px" borderColor="gray.200" bgColor="gray.50">
                <Box bg="gray.200" py="2">
                  <Center fontSize="lg" fontWeight="bold" textTransform="uppercase">
...
const ConfirmCompletePackageModal = ({ isOpen, onClose, packageDetail, orderDetail }) => {
  ...
  const handleCompletePackaging = async () => {
    ...
    try {
      setIsLoading(true);
      const res = await completePackage(cookies)(data);
      console.log({ res });
